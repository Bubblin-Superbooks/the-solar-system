! function($) {
				...
				self.transitionEnd = function(elem, callback) {
								...
								return elem && (event ? elem.bind(event, function() {
												...
												callback.call(elem);
								}, Math.ceil(1E3 * parseFloat(elem.css(getVendorPrefix() + "transition-duration"))))), event;
				};
				/**
				 * @param {Element} el
				 * @return {?}
				...
				if (os() !== 'windows') {
								var val = $.isTouch || 'ontouchstart' in window || navigator.MaxTouchPoints > 0 || navigator.msMaxTouchPoints > 0
				} else {
								var val = $.isTouch
				}

				/* OS specific windows detection */
				function os() {
